...
function parseVerseFromHtml(html: string, url: string): ParsedVerse | null {
  try {
    ...
    // Extract Sanskrit (Devanagari Unicode range U+0900–U+097F)
    const sanskritMatch = html.match(/([ऀ-ॿ]+(?:\s*।?[ऀ-ॿ]+)*)/)
...
if (verse) {
  console.log('=== PARSED VERSE: Bg 2.47 ===')
  ...
  console.log(`Translation excerpt word count: ${verse.translation_excerpt.split(/\s+/).length}`)
  console.log(`Translation excerpt: ${verse.translation_excerpt}`)
} else {
