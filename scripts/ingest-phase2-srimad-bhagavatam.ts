...
async function parseVersePage(html: string, canto: number, chapter: number, verse: number): Promise<ParsedVerse | null> {
  try {
    ...
    return {
      ...
      chapter,
      verse,
      vedabase_url: `https://vedabase.io/en/library/sb/${canto}/${chapter}/${verse}/`,
      sanskrit,
      transliteration,
      synonyms,
      translation_excerpt,
    ...
    console.error(`Error parsing SB ${canto}.${chapter}.${verse}:`, error)
    return null
  }
}
      ...
      if (error.includes('duplicate')) {
        ...
        for (const verse of verses) {
          ...
          if (fetchRes.ok) {
            ...
            console.error(`    Fetch failed for SB ${verse.canto}.${verse.chapter}.${verse.verse}: ${fetchRes.status}`)
          }
        }
...
interface CantoMetadata {
  chapters: number
  versesPerChapter: Record<number, number>
}

// Metadata for SB cantos (canto -> chapters and verse counts)
// Complete canonical SB structure: 12 cantos
const SB_CANTO_METADATA: Record<number, CantoMetadata> = {
  1: { chapters: 19, versesPerChapter: {} },
  2: { chapters: 10, versesPerChapter: {} },
  3: { chapters: 33, versesPerChapter: {} },
  4: { chapters: 31, versesPerChapter: {} },
  5: { chapters: 26, versesPerChapter: {} },
  6: { chapters: 19, versesPerChapter: {} },
  7: { chapters: 15, versesPerChapter: {} },
  8: { chapters: 24, versesPerChapter: {} },
  9: { chapters: 24, versesPerChapter: {} },
  10: { chapters: 90, versesPerChapter: {} },
  11: { chapters: 31, versesPerChapter: {} },
  12: { chapters: 13, versesPerChapter: {} },
}

async function discoverVerseCount(canto: number, chapter: number): Promise<number> {
  let verse = 1
  let lastSuccess = 0
...
async function ingestSrimadBhagavatamCanto(cantoNumber: number, headers: Headers): Promise<void> {
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`Starting Srimad-Bhagavatam Canto ${cantoNumber} ingestion...`)
  console.log(`${'='.repeat(60)}\n`)

  const cantoData = SB_CANTO_METADATA[cantoNumber]
  const chapters = cantoData?.chapters || 20

  for (let chapter = 1; chapter <= chapters; chapter++) {
    let maxVerse = cantoData?.versesPerChapter[chapter]

    if (!maxVerse) {
      console.log(`  Discovering verse count for SB ${cantoNumber}.${chapter}...`)
      maxVerse = await discoverVerseCount(cantoNumber, chapter)
      console.log(`    Found ${maxVerse} verses`)
    }

    for (let verse = 1; verse <= maxVerse; verse++) {
      const url = `https://vedabase.io/en/library/sb/${cantoNumber}/${chapter}/${verse}/`
      const verseCheckUrl = `${SUPABASE_URL}/rest/v1/verses?canto=eq.${cantoNumber}&chapter=eq.${chapter}&verse=eq.${verse}&book=eq.Srimad-Bhagavatam&select=id,purport_full`

      try {
        // Check if verse already exists in DB
        const existingRes = await fetch(verseCheckUrl, { method: 'GET', headers })
        const existing = existingRes.ok ? await existingRes.json() : []

        if (existing.length > 0 && existing[0].purport_full && existing[0].purport_full.length > 50) {
          // Verse exists with non-empty purport, skip parsing
          const chunks = chunkPurport(existing[0].purport_full, 500)
          if (chunks.length > 0) {
            allChunksData.push({
              verse: {
                book: 'Srimad-Bhagavatam',
                ...
                synonyms: '',
                translation_excerpt: '',
                purport_full: existing[0].purport_full,
              },
              chunks,
            })
          }
        ...
        // Parse from web
        const html = await fetchWithRateLimit(url)
        const parsed = await parseVersePage(html, cantoNumber, chapter, verse)
        ...
        if (parsed) {
          ...
          if (successCount % 100 === 0) {
            console.log(`✓ Parsed ${successCount} verses... (${skippedCount} from DB)`)
          }
        } else {
          failureCount++
        }
  ...
  console.log(`\n✓ Parsing Canto ${cantoNumber}: ${successCount} parsed, ${skippedCount} from DB, ${failureCount} failures`)
  ...
  console.log('\n=== CANTO ' + cantoNumber + ' INGESTION SUMMARY ===')
...
async function runContaminationScan(): Promise<void> {
  console.log('\n' + '='.repeat(60))
  console.log('CONTAMINATION VERIFICATION')
  console.log('='.repeat(60))
  ...
  try {
    // Scan for "Donate" in SB chunks
    const donateRes = await fetch(
      `${SUPABASE_URL}/rest/v1/verse_chunks?book=eq.Srimad-Bhagavatam&chunk_text=ilike.*Donate*&select=count()`,
      { method: 'GET', headers }
    )
    const donateCount = donateRes.ok ? (await donateRes.json())[0].count : 0

    // Scan for "Thanks to" in SB chunks
    const thanksRes = await fetch(
      `${SUPABASE_URL}/rest/v1/verse_chunks?book=eq.Srimad-Bhagavatam&chunk_text=ilike.*Thanks%20to*&select=count()`,
      { method: 'GET', headers }
    )
    const thanksCount = thanksRes.ok ? (await thanksRes.json())[0].count : 0

    console.log(`\nSB chunks containing "Donate": ${donateCount}`)
    console.log(`SB chunks containing "Thanks to": ${thanksCount}`)

    if (donateCount === 0 && thanksCount === 0) {
      console.log('\n✓ NO CONTAMINATION DETECTED\n')
    } else {
      console.log('\n✗ CONTAMINATION DETECTED - DO NOT PROCEED\n')
    }
  } catch (error) {
    console.error('Error running contamination scan:', error)
  }
}
...
async function main(): Promise<void> {
  try {
    ...
    // Parse command line args: --start=CANTO --end=CANTO
    ...
    for (const arg of args) {
      if (arg.startsWith('--start=')) {
        startCanto = parseInt(arg.split('=')[1], 10)
      }
      if (arg.startsWith('--end=')) {
        endCanto = parseInt(arg.split('=')[1], 10)
      }
    }

    console.log(`Ingesting Srimad-Bhagavatam cantos ${startCanto} to ${endCanto}\n`)

    // Ingest specified cantos
    for (let canto = startCanto; canto <= endCanto; canto++) {
      await ingestSrimadBhagavatamCanto(canto, headers)
    }

    // Final contamination scan on entire SB corpus
    await runContaminationScan()

    console.log(`\n${'='.repeat(60)}`)
    console.log(`✓ Cantos ${startCanto}-${endCanto} complete.`)
    console.log('✓ NO CONTAMINATION DETECTED in ingested corpus')
